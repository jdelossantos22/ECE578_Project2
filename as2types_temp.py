...
def as_links():
    ...
    #2.4 customer cone
    CustomerConeRanks = pd.DataFrame(columns=['AS','ASDegreee','CustomerCone','visited'])
    for AS in non_peers:
...
def immed_customers(CustomerConeRanks, non_peers, customers_deg, AS, CustomerCone):
    ...
    if currentASdegree == 0:
        newASrank = DataFrame([AS, 0, list(), True])
    ...
    if not AS in CustomerConeRanks:
        newASrank = DataFrame([AS, customerSubConeDegree, customerSubCone, True])
        CustomerConeRanks.append(newASrank)
